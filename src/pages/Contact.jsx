...
function ContactPage() {
  ...
  const submit = async (e) => {
    ...
    if (res.ok) { setStatus('Thanks! We will get back to you.'); setForm({ name: '', email: '', subject: '', message: '' }) } else { setStatus('Submission failed') }
  }
      ...
      <main className="pt-20">
        <section className="mx-auto max-w-3xl px-6 py-12">
          <div className="rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur">
            ...
            <p className="mt-2 text-white/80">Reach out for collaborations, partnerships and support.</p>
            <form onSubmit={submit} className="mt-6 space-y-3">
              ...
              <input name="subject" value={form.subject} onChange={onChange} placeholder="Subject" className="w-full rounded-lg bg-slate-900/60 px-3 py-2 border border-white/10" />
              <textarea name="message" value={form.message} onChange={onChange} placeholder="Message" rows="4" className="w-full rounded-lg bg-slate-900/60 px-3 py-2 border border-white/10" />
